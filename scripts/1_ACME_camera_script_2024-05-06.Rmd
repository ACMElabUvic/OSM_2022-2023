...
Let's first calculate the total number of days 
...
Now let's plot the camera operability with `ggplot()` to look at this
...
```{r}
ggsave('OSM_cam_op_2022_2023.jpg',
       cam_op,
       path = 'figures',
       width = 14,
       height = 12,
...
We should also remove the messy data from the environment so we don't accidentally use it
...
# total images
# see observations in global environment

# total images with something in them
OSM_2022_data_fixed %>% 
  
  drop_na(species) %>% 
  
  summarise(non_blank_images = n())

# images of wildlife id to species
mammals_2022 <- OSM_2022_data_fixed %>% 
  
  filter(species %in% c('White-tailed deer',
                        'Black bear',
                        'Snowshoe hare',
                        'Moose',
                        'Coyote',
...
mammals_2022 %>% 
  ...
  summarise(images_of_mammals_to_species = n())
  

# species image summaries
mammals_2022 %>% 
