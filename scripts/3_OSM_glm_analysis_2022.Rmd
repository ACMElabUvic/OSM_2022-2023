...
It's also relevant to wonder if these results would be different depending on if we look at exclusively disturbance or landscape features in a model and by reducing the variables in each model we can address this issue of autocorrelation much easier. So the next analysis will repeat what we've done above for each species but instead of using one global model for everything we will divide the data into disturbance and landscape covariates and run two separate models per species and see if the results differ. 
...
osm_developed_df_2021_2022 <- osm_final_df_2021_2022 %>% 
...
Let's do this for the developed data first

```{r correlation plots developed pearson, warning=FALSE}

correlation_plots <- osm_developed_df_2021_2022 %>% 
...
We'll reformat the developed subset data here and re-run the correlation plots

```{r reformat dev subset data}

osm_developed_df_2021_2022 <- osm_developed_df_2021_2022 %>% 
...
```{r re-run corr plots dev data, warning=FALSE}
...
correlation_plots <- osm_landscape_df_2021_2022 %>% 
...
We will do ana analysis for each species as we did previously.
...
bear_dev_mods <- osm_developed_df_2021_2022 %>% 
...
```{r bear dev mod selection}

model.sel(bear_dev_mods)
...
summary(bear_dev_mods$`4000 meter buffer`)
```
...
test_bear_dev_mods <- osm_developed_df_2021_2022 %>% 
...
model.sel(test_bear_dev_mods)


```
> Interesting, you get almost the same results  but the 4000m buffer is the worst fit. There was a warning about model convergence issues> I wonder why that would be?
