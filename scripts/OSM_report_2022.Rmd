...
str(covaraites_grouped)

# check summary of new data
summary(covaraites_grouped)
...
covaraites_grouped <- covaraites_grouped %>% 
...
Let's make correlation plots for these variables, we can use purrr to make them all with one section of code
...
```
Now we can save all these correlation plots using ggsave?

#ANDREW HELP

I need to save all these plots but they are saved as a different kind of object in the list than the ggplots I made earlier were and so purrr::imap and ggsave don't like them. Do you have code to save all the correlation plots I generated above?
...
#Couldn't get it to work with the buffer_frames list, was giving me grief. So I took it back to the orriginal covaraites_grouped frame
#subset the data to the current buff_dist
#I couldn't figure out how to access the correlation plots from the list, so here I just save them on the fly with the png and dev.off pair
#You could modify this to save to a list as well if you wanted though
