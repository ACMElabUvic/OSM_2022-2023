...
Let's also save this graph in case we want to look at it later without running all the code
...
We'll start with the deployment data (deploy)
...
Ssomething weird is happening with the end of one of the LU15 sites it has a later end date than the others by a lot. Let's plot just LU15 so we can see which site it is, it is probably a typo in the deployment data we will need to fix in the raw file.
...
We may want to save the plot for easier viewing later

```{r save plot total ind det}
...
And we save the graph to view easier later on

```{r save plot presence/absence}
